          ...
          if (method.toLocaleLowerCase() === "get") {
            ...
            for (const key of Object.keys(wrapperData)) {
              if (typeof wrapperData[key] === "object" && Reflect.has(wrapperData[key], "handler")) {
                ...
                const result = await handler2(env);
...
`;
  ...
  function optimizationElementWithImage(root2, clone_node = false) {
    ...
    const sourceElements = [root2, ...Array.from(root2.querySelectorAll("*"))];
    ...
    for (const img of Array.from(clone.querySelectorAll("img"))) {
      if (Array.from(img.parentElement.querySelectorAll("span")).some(
        (e) => {
          var _a2;
          return e.style.fontSize === "0px" && ((_a2 = e.textContent) == null ? void 0 : _a2.includes(img.src));
        }
      )) {
        continue;
      }
    ...
    for (let i = clonedElements.length - 1; i >= 1; i--) {
      ...
      if (!source || !target || !target.isConnected && !target.parentNode)
        continue;
      let style;
      try {
      ...
      const blankInput = (source.tagName === "TEXTAREA" || source.getAttribute("contenteditable") === "true" || source.tagName === "INPUT") && !["hidden", "radio", "checkbox", "button", "submit"].includes(inputType);
      const bottomOnly = !!style && parseFloat(style.borderBottomWidth || "0") > 0 && parseFloat(style.borderTopWidth || "0") === 0 && parseFloat(style.borderLeftWidth || "0") === 0 && parseFloat(style.borderRightWidth || "0") === 0;
      const underlined = ["U", "INS"].includes(source.tagName) || /underline|under-line/.test(className) || /text-decoration(?:-line)?\s*:\s*underline/.test(inlineStyle) || ((_a = style == null ? void 0 : style.textDecorationLine) == null ? void 0 : _a.includes("underline")) || bottomOnly;
      if (!underlined && !blankInput)
        continue;
      const text = blankInput ? "" : String(target.textContent || "").trim();
      const marker = text ? ` [UNDERLINE]${text}[/UNDERLINE] ` : ` [BLANK_${++blankIndex}] `;
    ...
    for (const node of textNodes) {
      node.nodeValue = String(node.nodeValue || "").replace(
        ...
        (_matched, open, close) => open ? `${open}[BLANK_${++blankIndex}]${close}` : ` [BLANK_${++blankIndex}] `
      );
    }
    return clone;
  }
        ...
        configs: {
          ...
          thread: {
            ...
            attrs: {
              ...
              min: 1,
              step: 1,
              title: "同一时间内答题线程工作的数量，不设上限。每个线程都可能发起一次 AI 请求，请根据服务器和 API 并发能力设置。"
            },
            defaultValue: 1
          },
          "work-when-no-job": {
...
主页：${c.homepage || "未知主页"}`,
  ...
  function getCXQuestionType(value, label = "") {
  ...
  function getCXNativeType(root2) {
    const input = root2.querySelector(
      'input[name^="answertype"],input[id^="answertype"],input[name^="type"]'
    );
    return String(
      (input == null ? void 0 : input.value) || root2.getAttribute("data-type") || (root2.matches(".TiMu") ? root2.getAttribute("data") : "") || ""
    ).trim();
  }
  function resolveCXQuestionType(root2, label = "", explicitNativeType = "") {
    const nativeType = String(explicitNativeType != null ? explicitNativeType : "").trim() || getCXNativeType(root2);
    return getCXQuestionType(Number.parseInt(nativeType || "-1"), label);
  }
  function normalizeAnswerItem(value) {
  ...
  function parseAnswer(value, separators) {
  ...
  function getCXAnswerCandidates(searchInfos, separators = []) {
    const actualSeparators = separators.length ? separators : ["===", "###", "---", "#", "|", ";", "；"];
    return searchInfos.flatMap(
      (info) => info.results.map((result) => String(result.answer || "").trim()).filter(Boolean).map((answer) => parseAnswer(answer, actualSeparators))
    );
  }
  function answerValueList(value) {
    const normalized = normalizeAnswerItem(value);
    if (Array.isArray(normalized))
      return normalized.map(normalizeAnswerItem);
  ...
  function uniqueElements(elements2) {
    return elements2.filter((element, index) => elements2.indexOf(element) === index);
  }
  ...
  function getUEditorLogicalTarget(control, root2) {
    const isEditorFrame = control.matches(ueditorFrameSelector) || control.matches("iframe") && !!control.closest(".edui-editor,.edui-editor-iframeholder");
    const isBackingTextarea = control.matches(ueditorTextareaSelector);
    if (!isEditorFrame && !isBackingTextarea)
      return;
    let container2 = control.parentElement;
    while (container2 && root2.contains(container2)) {
      const frames = container2.querySelectorAll(ueditorFrameSelector);
      const textareas = container2.querySelectorAll(ueditorTextareaSelector);
      if (frames.length === 1 && textareas.length === 1)
        return container2;
      if (container2 === root2)
        break;
  ...
  function getCXCompletionTargets(root2, configured = []) {
    const controls = [
      ...configured,
      ...Array.from(
    ...
    for (const control of controls) {
      const editorGroup = getUEditorLogicalTarget(control, root2);
      if (editorGroup) {
        targets.push(editorGroup);
        continue;
      }
      const group = control.closest(".Briefanswer[data-itemid],.blankItem,.jdt,.textDIV,.eidtDiv,.editDiv");
      if (group && root2.contains(group))
        targets.push(group);
      else if (root2.contains(control) || control === root2)
  ...
  function isCXQuestionReadOnly(root2) {
    const text = root2.innerText || root2.textContent || "";
    if (!/(我的答案|已作答|已回答|参考答案|答案解析)/.test(text))
      return false;
    return !hasCXEditableControls(root2);
  }
  function hasCXEditableControls(root2) {
    return !!root2.querySelector(cxEditableControlSelector);
  }
  function dispatchValueEvent(target, type) {
    var _a;
    const document2 = target instanceof Node ? target.ownerDocument : void 0;
    const EventConstructor = ((_a = document2 == null ? void 0 : document2.defaultView) == null ? void 0 : _a.Event) || Event;
    target.dispatchEvent(new EventConstructor(type, { bubbles: true }));
  }
  function fillCXCompletionTarget(target, answer) {
    var _a, _b, _c, _d, _e, _f;
    let filled = false;
    const textarea = target.matches('textarea,input[type="text"],input:not([type])') ? [target] : Array.from(target.querySelectorAll('textarea,input[type="text"],input:not([type])'));
    const editorIndex = Number(
      target.getAttribute("data-editorindex") || ((_a = target.querySelector("[data-editorindex]")) == null ? void 0 : _a.getAttribute("data-editorindex")) || ((_b = textarea[0]) == null ? void 0 : _b.getAttribute("step"))
    );
    const editors = ((_c = target.ownerDocument.defaultView) == null ? void 0 : _c.editors) || globalThis.editors;
    const editor = Number.isFinite(editorIndex) ? ((_d = editors == null ? void 0 : editors[editorIndex]) == null ? void 0 : _d.ueditor) || (editors == null ? void 0 : editors[editorIndex]) : void 0;
    if (editor == null ? void 0 : editor.setContent) {
      editor.setContent(answer);
      (_e = editor.fireEvent) == null ? void 0 : _e.call(editor, "contentChange");
      filled = true;
    }
    const frames = target.matches("iframe") ? [target] : Array.from(target.querySelectorAll("iframe"));
    for (const frame of frames) {
      const body = (_f = frame.contentDocument) == null ? void 0 : _f.body;
      if (!body)
        continue;
    ...
    const editables = target.matches('[contenteditable="true"]') ? [target] : Array.from(target.querySelectorAll('[contenteditable="true"]'));
  ...
  function directOptionText(element, render2) {
    return cleanText(render2(element));
  }
  function getCXQuestionDetails(root2, title, nativeType, render2) {
    const materials = Array.from(
      root2.querySelectorAll('.material,.question-material,.case,[class*="material"]')
    ).map((element) => directOptionText(element, render2)).filter((text, index, all) => !!text && text !== title && all.indexOf(text) === index);
    const expectedMatchingCount = root2.querySelectorAll(
      ".thirdUlList .dept_select,.line_answer_ct .selectBox,.selLineList > li"
    ).length;
    const matchingGroup = (primarySelector, fallbackSelector) => {
      const primary = Array.from(root2.querySelectorAll(primarySelector));
      const nodes = primary.length ? primary : Array.from(root2.querySelectorAll(fallbackSelector));
      const values = nodes.map((element) => directOptionText(element, render2)).filter(Boolean);
      return primary.length && expectedMatchingCount && values.length === expectedMatchingCount + 1 ? values.slice(1) : values;
    };
    ...
    const sharedOptions = Array.from(
      root2.ownerDocument.querySelectorAll(
    ...
    ).filter((element) => !element.closest(".questionLi,.singleQuesId,.TiMu") || root2.contains(element)).map((element) => directOptionText(element, render2)).filter((text, index, all) => !!text && all.indexOf(text) === index);
    const subquestions = [];
    const childTypes = Array.from(root2.querySelectorAll('input[name="readCompreHension-childType"]'));
    for (let index = 0; index < childTypes.length; index++) {
      const typeInput = childTypes[index];
      const idInput = typeInput.nextElementSibling;
      let container2 = idInput == null ? void 0 : idInput.nextElementSibling;
      while (container2 && container2.tagName !== "UL" && !container2.matches(".reading_answer,.filling_answer")) {
        container2 = container2.nextElementSibling;
      }
      if (!container2)
        continue;
      const promptNode = container2.querySelector(".ignoreli .ans-cc,.ignoreli,.question-title,.title");
      const options = Array.from(container2.querySelectorAll("li:not(.ignoreli),.answer_p,.option")).map((element) => directOptionText(element, render2)).filter(Boolean);
      subquestions.push({
        id: String((idInput == null ? void 0 : idInput.value) || index + 1),
        index: index + 1,
        type: getCXQuestionType(Number(typeInput.value), (promptNode == null ? void 0 : promptNode.innerText) || ""),
        title: promptNode ? directOptionText(promptNode, render2) : `子题 ${index + 1}`,
        options
      });
    }
    if (subquestions.length === 0) {
      const groups = Array.from(
        root2.querySelectorAll(".reading_answer,.filling_answer,.clozeBlank,.B-answerCon")
      );
      for (let index = 0; index < groups.length; index++) {
        const group = groups[index];
        const prompt = group.querySelector(".ans-cc,.question-title,.title,.ignoreli");
        const options = Array.from(
          group.querySelectorAll("span.saveSingleSelect,.choice,li:not(.ignoreli),span[data]")
        ).map((element) => directOptionText(element, render2)).filter(Boolean);
        subquestions.push({
          id: String(group.getAttribute("data-itemid") || index + 1),
          index: index + 1,
          type: "single",
          title: prompt ? directOptionText(prompt, render2) : `子题 ${index + 1}`,
          options
        });
      }
    }
    return {
      ...
      blank_count: Math.max((title.match(/\[BLANK_\d+\]/gi) || []).length, getCXCompletionTargets(root2).length),
  ...
  function normalizeComparable(value) {
    return String(value || "").replace(/^[A-Z0-9]+[.、:：)）\-\s]+/i, "").replace(/[\s、,，;；:：.．()（）]/g, "").toLocaleLowerCase();
  }
  function clickCXChoice(container2, answer) {
    ...
    const exact = candidates.find((candidate) => {
      ...
      const values = [
        ...
        (_a = candidate.querySelector("em")) == null ? void 0 : _a.getAttribute("id-param"),
        (_b = candidate.querySelector("em")) == null ? void 0 : _b.innerText,
        candidate.innerText
      ];
      return values.some((value) => normalizeComparable(value) === wanted);
    });
    const similar = exact || candidates.find((candidate) => {
  ...
  const cxCompoundQuestionTypes = ["cloze", "reading", "listening", "shared_options", "composite"];
  let activeCXChapterRestartEvent;
  function renderCXElement(element) {
    return optimizationElementWithImage(element, true).innerText.trim();
  }
  function cxAnswerText(value) {
    ...
    if (Array.isArray(value))
      return value.map(cxAnswerText).filter(Boolean).join(" ");
    if (typeof value === "object") {
      const item = value;
      for (const key of ["right", "answer", "text", "value", "option", "content"]) {
        if (key in item)
          return cxAnswerText(item[key]);
      }
    }
    return "";
  }
  function createCXAnswerPayload(root2, title, type, options) {
    const nativeType = getCXNativeType(root2);
    const details = getCXQuestionDetails(root2, title, nativeType, renderCXElement);
  ...
  function getCXVisibleTypeLabel(titles) {
    return titles.map((title) => (title == null ? void 0 : title.innerText) || (title == null ? void 0 : title.textContent) || "").join("\n");
  }
  function searchCXAnswer(title, type, search) {
    if (cxCompoundQuestionTypes.includes(type) || type === "matching" || type === "ordering")
      return search();
    return CommonProject.scripts.apps.methods.searchAnswerInCaches(title, search);
  }
  async function fillCXTextQuestion(searchInfos, root2, configuredTargets, answerSeparators = []) {
    var _a;
    const targets = getCXCompletionTargets(root2, configuredTargets);
    if (targets.length === 0) {
      if (isCXQuestionReadOnly(root2)) {
        $console.info("检测到题目为只读，等待手动进入修改答案后再运行。");
        return { finish: true, skipped: true };
      }
      throw new Error("已获取主观题/填空题答案，但未找到可填写的编辑框。");
    }
    const candidateSeparators = targets.length === 1 ? ["\0"] : answerSeparators;
    for (const candidate of getCXAnswerCandidates(searchInfos, candidateSeparators)) {
      ...
      if (answers.length === 0)
        continue;
      if (targets.length === 1 && answers.length > 1)
        answers = [answers.join(" ")];
      if (answers.length !== targets.length)
        continue;
      let filled = 0;
      for (let index = 0; index < targets.length; index++) {
        if (fillCXCompletionTarget(targets[index], answers[index]))
          filled++;
      }
      if (filled === targets.length) {
        (_a = lib.$el("[onclick*=saveQuestion]", root2)) == null ? void 0 : _a.click();
        await $.sleep(300);
        return { finish: true };
      }
    }
  ...
  function getCXCompoundRows(root2, fallback = []) {
    var _a;
    const rows = [];
    for (const typeInput of Array.from(
      root2.querySelectorAll('input[name="readCompreHension-childType"]')
    )) {
      let row = (_a = typeInput.nextElementSibling) == null ? void 0 : _a.nextElementSibling;
    ...
    if (rows.length === 0) {
      rows.push(
        ...Array.from(root2.querySelectorAll(".clozeBlank,.B-answerCon,.reading_answer,.filling_answer"))
      );
    }
    if (rows.length === 0)
      rows.push(...fallback);
  ...
  async function fillCXCompoundQuestion(searchInfos, root2, fallbackRows = [], answerSeparators = []) {
    const rows = getCXCompoundRows(root2, fallbackRows);
    if (rows.length === 0) {
      return getCXCompletionTargets(root2).length ? fillCXTextQuestion(searchInfos, root2, [], answerSeparators) : { finish: false };
    }
    for (const candidate of getCXAnswerCandidates(searchInfos, answerSeparators)) {
      ...
      for (let index = 0; index < rows.length; index++) {
        ...
        if (textTargets.length) {
          ...
          if (texts.length !== textTargets.length)
            break;
          ...
          if (!choices.every((answer) => clickCXChoice(row, answer)))
            break;
          completed++;
        }
        await $.sleep(150);
  ...
  async function fillCXMatchingQuestion(searchInfos, root2, selectBoxes = [], answerSeparators = []) {
    const nativeRows = Array.from(root2.querySelectorAll(".selLineList > li"));
    const rows = nativeRows.length ? nativeRows : selectBoxes;
    if (rows.length === 0)
      return { finish: false };
    let rightItems = Array.from(
      root2.querySelectorAll(".secondUlList > li,.answerList-line:nth-of-type(2) > li")
    ).map(renderCXElement);
    if (rightItems.length === rows.length + 1 && root2.querySelector(".secondUlList"))
      rightItems = rightItems.slice(1);
    const normalize = (value) => value.replace(/[\s、,，;；:：.．()（）]/g, "").toLocaleLowerCase();
    const answerForControl = (answer) => {
    ...
    for (const candidate of getCXAnswerCandidates(searchInfos, answerSeparators)) {
      ...
      for (let index = 0; index < rows.length; index++) {
        ...
        if (select) {
          ...
          const customOption = row.querySelector(`li[data="${CSS.escape(answers[index])}"] a`);
          if (!customOption)
            break;
          customOption.click();
          matched++;
  ...
  async function fillCXOrderingQuestion(searchInfos, root2, answerSeparators = []) {
    const rows = Array.from(root2.querySelectorAll(".sortQuesSelect > li,.sortQuesSelect li"));
    if (rows.length === 0)
      return { finish: false };
    for (const candidate of getCXAnswerCandidates(searchInfos, answerSeparators)) {
      ...
      let matched = 0;
      for (let index = 0; index < rows.length; index++) {
        if (!clickCXChoice(rows[index], answers[index]))
          break;
        matched++;
        await $.sleep(150);
      }
  ...
  function workOrExam$1(type = "work", {
    ...
    const worker = new OCSWorker({
      ...
      answerer: (elements2, ctx) => {
        if (elements2.title) {
          ...
          if (title) {
            ...
            const type2 = resolveCXQuestionType(ctx.root, getCXVisibleTypeLabel(elements2.title), typeInput == null ? void 0 : typeInput.value);
            if (type2 === "poll" || type2 === "oral" || type2 === "oral_evaluation") {
              throw new Error("此题需要投票或录音作答，OCS 不会伪造答案或误报完成。");
            }
            return searchCXAnswer(title, type2, async () => {
              await $.sleep((period != null ? period : 3) * 1e3);
              return defaultAnswerWrapperHandler(
                answererWrappers,
                createCXAnswerPayload(ctx.root, title, type2, ctx.elements.options)
              );
            });
          } else {
            throw new Error("题目为空，请查看题目是否为空，或者忽略此题");
      ...
      work: async (ctx) => {
        ...
        const nativeType = (_a = elements2.type.find((t2) => {
          var _a2;
          return (_a2 = t2.getAttribute("name")) == null ? void 0 : _a2.match(/(?:answer)?type\d+/);
        })) == null ? void 0 : _a.getAttribute("value");
        const type2 = resolveCXQuestionType(ctx.root, getCXVisibleTypeLabel(elements2.title), nativeType);
        if (type2 === "multiple" || type2 === "judgement" || type2 === "single" || type2 === "evaluation") {
          const choiceType = type2 === "evaluation" ? "multiple" : type2;
        ...
        if (cxTextQuestionTypes.includes(type2)) {
          return fillCXTextQuestion(searchInfos, ctx.root, elements2.options, ctx.answerSeparators);
        }
        if (type2 === "matching") {
          return fillCXMatchingQuestion(searchInfos, ctx.root, elements2.lineSelectBox, ctx.answerSeparators);
        }
        if (cxCompoundQuestionTypes.includes(type2)) {
          return fillCXCompoundQuestion(
            searchInfos,
            ctx.root,
            [...elements2.filling, ...elements2.reading],
  ...
  async function study(opts) {
    ...
    await runJobs();
    if (chapterRestartPending) {
      const msg2 = "当前章节测试处于只读状态，请先点击题目页面的“修改答案”，再点击 OCS 面板中的“重新答题”。";
        ...
        if (attachment && searchedJobs.find((job2) => job2.mid === attachment.property.mid) === void 0) {
          ...
          if (videojs) {
            ...
            if (!CXProject.scripts.study.cfg.enableChapterTest) {
              ...
              if ((status == null ? void 0 : status.classList.contains("testTit_status_complete")) || /已完成/.test((status == null ? void 0 : status.innerText) || "")) {
  ...
  const JobRunner = {
    ...
    async chapter(frame, options, showRestartButton = true) {
      const {
        answererWrappers,
        period,
        upload,
      ...
      if (showRestartButton) {
        ...
        const restartChapter = async () => {
          ...
          const liveRoots = Array.from(frameWindow.document.querySelectorAll(".TiMu"));
          if (liveRoots.filter((root2) => !isCXQuestionReadOnly(root2)).length === 0) {
            return {
              started: false,
          ...
          (_a = workerRef.current) == null ? void 0 : _a.emit("close");
          await $.sleep(100);
          try {
        ...
        await lib.cors.on(restartEventName, async () => {
          try {
            return await restartChapter();
          } catch (error) {
            const message2 = error instanceof Error ? error.message : String(error);
            $console.error("手动重新答题失败", error);
            return { started: false, message: `重新答题失败：${message2}` };
          }
        });
      }
      ...
      if (restartEventName) {
        await CORSUtils.showChapterRestartButton(
          restartEventName,
          initialTiMu.length > 0 && initialTiMu.every((root2) => isCXQuestionReadOnly(root2))
        );
      }
      const TiMu = initialTiMu.filter((root2) => !isCXQuestionReadOnly(root2));
      if (TiMu.length === 0) {
      ...
      const chapterWorker = new OCSWorker({
        ...
        answerer: (elements2, ctx) => {
          ...
          if (title) {
            const typeInput = elements2.type[0];
            const type = resolveCXQuestionType(ctx.root, getCXVisibleTypeLabel(elements2.title), typeInput == null ? void 0 : typeInput.value);
        ...
        work: async (ctx) => {
          ...
          if (type === "multiple" || type === "judgement" || type === "single" || type === "evaluation") {
            ...
            const handler = (type2, answer, option, ctx2) => {
              var _a, _b, _c;
              const checked = ((_b = (_a = option == null ? void 0 : option.parentElement) == null ? void 0 : _a.querySelector("label input")) == null ? void 0 : _b.getAttribute("checked")) === "checked" || ((_c = option == null ? void 0 : option.parentElement) == null ? void 0 : _c.getAttribute("aria-checked")) === "true";
              if (!checked) {
        ...
        async onResultsUpdate(curr, _, res) {
          ...
          if (((_b = curr.result) == null ? void 0 : _b.finish) === false && curr.resolved === true) {
            const options2 = ((_d = (_c = curr.ctx) == null ? void 0 : _c.elements) == null ? void 0 : _d.options) || [];
            const typeInput = (_f = (_e = curr.ctx) == null ? void 0 : _e.elements) == null ? void 0 : _f.type[0];
            const type = resolveCXQuestionType(
              curr.ctx.root,
              getCXVisibleTypeLabel(((_h = (_g = curr.ctx) == null ? void 0 : _g.elements) == null ? void 0 : _h.title) || []),
              typeInput == null ? void 0 : typeInput.value
            );
            const commonSetting = CommonProject.scripts.settings.cfg;
            if (commonSetting["randomWork-choice"] && (type === "judgement" || type === "single" || type === "multiple" || type === "evaluation")) {
              $console.log("正在随机作答");
              const option = options2[Math.floor(Math.random() * options2.length)];
              (_j = (_i = option == null ? void 0 : option.parentElement) == null ? void 0 : _i.querySelector("a,label")) == null ? void 0 : _j.click();
            } else if (commonSetting["randomWork-complete"] && cxTextQuestionTypes.includes(type)) {
              $console.log("正在随机作答");
              for (const option of getCXCompletionTargets(curr.ctx.root, options2)) {
        ...
        async onElementSearched(elements2) {
          var _a, _b, _c, _d;
          const typeInput = elements2.type[0];
          const questionRoot = (typeInput == null ? void 0 : typeInput.closest(".TiMu")) || ((_b = (_a = elements2.title) == null ? void 0 : _a[0]) == null ? void 0 : _b.closest(".TiMu"));
          if (!questionRoot)
            return;
          const type = resolveCXQuestionType(questionRoot, ((_d = (_c = elements2.title) == null ? void 0 : _c[0]) == null ? void 0 : _d.innerText) || "", typeInput == null ? void 0 : typeInput.value);
          if (type === "judgement") {
            elements2.options.forEach((option) => {
              var _a2;
              const opt = ((_a2 = option == null ? void 0 : option.textContent) == null ? void 0 : _a2.trim()) || "";
      ...
      await chapterWorker.uploadHandler({
        ...
        async callback(finishedRate, uploadable) {
          ...
          await $.sleep(3e3);
          if (uploadable) {
            if (typeof frameApi.btnBlueSubmit === "function")
              frameApi.btnBlueSubmit();
            else if (typeof frameApi.submitHomework === "function")
              frameApi.submitHomework();
            await $.sleep(3e3);
            if (typeof frameApi.submitCheckTimes === "function")
              frameApi.submitCheckTimes();
            top$1.$("#workpop").hide();
          } else {
            frameWindow.alert = () => {
            };
            if (typeof frameApi.noSubmit === "function")
              frameApi.noSubmit();
            else if (typeof frameApi.saveAnswer === "function")
              frameApi.saveAnswer();
            else {
              const saveButton = frameApi.document.querySelector(
                '[onclick*="save"],button:not([type="submit"]),input[type="button"]'
              );
              saveButton == null ? void 0 : saveButton.click();
            }
          }
        }
      });
      ...
      if (restartEventName) {
        ...
        lib.cors.off(restartEventName);
        if (activeCXChapterRestartEvent === restartEventName)
          activeCXChapterRestartEvent = void 0;
      }
      return "completed";
    },
  ...
  const CORSUtils = {
    ...
    showChapterRestartButton: lib.cors.defineTopFunction((eventName, requiresEdit = false) => {
      let cancelled = false;
      const install = (attempt = 0) => {
        ...
        if (!panel) {
          if (attempt < 20)
            setTimeout(() => install(attempt + 1), 100);
          return;
        }
        if (panel.body.querySelector(`[data-ocs-cx-chapter-restart="${eventName}"]`))
          return;
        (_b = (_a = panel.body.querySelector("[data-ocs-cx-chapter-restart]")) == null ? void 0 : _a.parentElement) == null ? void 0 : _b.remove();
        const button = lib.$ui.button("🔃重新答题", {
          className: "base-style-button",
          style: { flex: "1", padding: "4px" }
        });
        button.title = "请先在题目页面点击“修改答案”，再点击此按钮重新检测题目并答题。";
        button.setAttribute("data-ocs-cx-chapter-restart", eventName);
        const container2 = lib.h("div", { style: { margin: "12px 0", display: "flex", gap: "8px" } }, [
          button,
          lib.h("span", requiresEdit ? "请先点击题目页面的“修改答案”" : "重新检测当前页面题目并答题")
        ]);
        button.onclick = () => {
          cancelled = true;
          button.disabled = true;
          button.value = "⏳重新检测中...";
          lib.cors.emit(eventName, [], (result) => {
            if (result == null ? void 0 : result.started) {
              container2.remove();
              return;
            }
            ...
            button.value = "🔃重新答题";
            lib.$message.warn({
              content: (result == null ? void 0 : result.message) || "仍未检测到可填写的编辑框，请先点击题目页面的“修改答案”。",
              duration: 5
            });
          });
        };
        panel.body.append(container2);
        setTimeout(() => {
    ...
    removeChapterRestartButton: lib.cors.defineTopFunction((eventName) => {
      var _a, _b;
      const button = Array.from(((_a = CommonProject.scripts.workResults.panel) == null ? void 0 : _a.body.querySelectorAll("input")) || []).find(
        (element) => element.getAttribute("data-ocs-cx-chapter-restart") === eventName
      );
      (_b = button == null ? void 0 : button.parentElement) == null ? void 0 : _b.remove();
    }),
