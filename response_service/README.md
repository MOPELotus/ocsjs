...
更新到带复合题支持的版本后，也要重新复制一次完整题库配置。新版配置除题干和普通选项外，还会提交材料、子题、共用选项、匹配分组、原生题型编号以及挖空/下划线计数；沿用旧配置仍可回答普通题，但复合题会缺少对应关系。

当前自动回填覆盖单选、多选、判断、填空、简答/名词解释/论述、计算、分录、连线/匹配、排序、完形填空、阅读理解、听力复合题、共用选项、资料/复合题、测评选择题和写作题。复合题只有在每个子题都实际匹配并回填成功时才标记完成。投票和需要录音的口语题会明确保留为手动作答，不会伪造答案或误报完成。
